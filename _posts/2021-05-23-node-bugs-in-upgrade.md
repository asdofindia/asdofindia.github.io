---
layout: post
title: "It Was Working In Previous Release, But Not On In The Present - Debugging Bugs Introduced By Dependency Upgrades"
...
With the command `npm install`, there is a tricky situation though (as described above). Imagine version 1.4.0 of a library has a bug. Imagine two months ago the library we installed was 1.3.2 and we specified the version range as ^1.3.2. Now if we do `npm install` on our newest commit, it'll pull the latest version (1.4.0 or later) including the bug. So searching through to git history to find which commit introduced a bug we will go back to our code two months ago. We do `npm install`. And without any qualms, npm will install 1.4.0 or later and the bug. And we will know no better.
